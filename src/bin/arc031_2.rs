...
impl Solver {
    fn dfs(&mut self, pos: (i32, i32)) {
        self.seen.insert(pos);

        for &d in DIRECTIONS.iter() {
            let pos_new = (pos.0 + d.0, pos.1 + d.1);
            if pos_new.0 < 0 || pos_new.0 >= self.board.len() as i32 { continue; }
            if pos_new.1 < 0 || pos_new.1 >= self.board[0].len() as i32 { continue; }
            if self.board[pos_new.0 as usize][pos_new.1 as usize] == 'x' { continue; }
...
fn main() {
    ...
    for i in 0..10 {
        for j in 0..10 {
            ...
            solver.dfs((i as i32, j as i32));

            if solver.seen.len() == total + 1 {
                println!("YES");
                return;
            }
        }
