...
fn main() {
    ...
    for i in 0..H {
        for j in 0..W {
            if board[i][j] == 's' {
                pos_s = (i as i32, j as i32);
                board[i][j] = '.';
            }
            if board[i][j] == 'g' {
                pos_g = (i as i32, j as i32);
                board[i][j] = '.';
            }
        }
    }
    ...
    while let Some(src) = queue.pop_front() {
        for &d in DIRECTIONS.iter() {
            ...
            if costs.contains_key(&dst) { continue; }

            let cost = if board[dst.0 as usize][dst.1 as usize] == '.' {
