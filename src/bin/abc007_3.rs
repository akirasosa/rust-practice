...
fn main() {
    ...
    let R: usize = R;
    let C: usize = C;
    let pos_s: (i32, i32) = (pos_s.0 - 1, pos_s.1 - 1);
    let pos_g: (i32, i32) = (pos_g.0 - 1, pos_g.1 - 1);
    let board: Vec<Vec<char>> = board;
//    debug!(R, C, pos_s, pos_g, board);
    ...
    while let Some(pos) = queue.pop_front() {
        for &d in DIRECTIONS.iter() {
            ...
            if pos_new.0 < 0 || pos_new.0 >= R as i32 { continue; }
            if pos_new.1 < 0 || pos_new.1 >= C as i32 { continue; }
