...
impl Solver {
    fn dfs(&mut self, v: usize) {
        ...
        for v_next in self.G[v].to_vec() {
            let (v_tmp, u_tmp) = if v < v_next {
                (v, v_next)
            } else {
                (v_next, v)
            };
            let visited = self.seen.contains(&v_next);
            *self.log.entry((v_tmp, u_tmp)).or_insert(0) += visited as usize;
...
fn main() {
    ...
    for v in 0..N {
        if solver.seen.contains(&v) { continue; }
        solver.clear_log();
        solver.dfs(v);

        if solver.log.values().all(|&x| x <= 1) {
            res += 1;
        }
    }
