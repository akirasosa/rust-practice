...
fn main() {
    input! {
        N: usize,
        M: usize,
        aa: [(usize, usize, usize, usize); N],
        bb: [(usize, usize, usize); M],
    }
    let mut aa: Vec<(usize, usize, usize, usize)> = aa;
    let bb: Vec<(usize, usize, usize)> = bb;
//    debug!(N, M, aa, bb);

    let mut dp = vec_md![0; 102; 102; 102];

    aa.sort_by_key(|a| a.3);
    for a in aa {
        dp[a.0][a.1][a.2] = a.3;
    }
