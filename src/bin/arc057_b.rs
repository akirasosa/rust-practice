...
//    debug!(N, K, aa);
    ...
    // dp[i][j] : i 日までに j 回機嫌が良くなっているための、必要最低限の累積勝利数
    let mut dp: Vec<Vec<usize>> = vec_md![K + 1; N + 1; N + 1];
    for i in 0..N {
        dp[i][0] = 0;
    }
    dp[0][1] = 1;
    ...
    for i in 1..N {
        for j in 1..i + 2 {
            // 機嫌が良くならない -> 一度も勝たなくて良い (累積勝利数は昨日と同じ)
            dp[i][j] = min(dp[i][j], dp[i - 1][j]);

            // 機嫌が良くなる -> 最低限必要な累積勝利数を求める
            let x = dp[i - 1][j - 1] * bb[i] / bb[i - 1] + 1;
            if x - dp[i - 1][j - 1] > aa[i] { continue; }
