...
fn rem_euclid(a: i64, rhs: i64) -> usize {
    let r = a % rhs;
    let res = if r < 0 {
        if rhs < 0 {
            r - rhs
...
fn div_euclid(a: i64, rhs: i64) -> usize {
    let q = a / rhs;
    if a % rhs < 0 {
        return if rhs > 0 { q - 1 } else { q + 1 } as usize;
    }
    q as usize
}
...
fn main() {
    ...
    for i in 0..N {
        for j in 0..M {
            let j_pre = rem_euclid(j as i64 - aa[i] as i64, M as i64);
            dp[i + 1][j] = min(dp[i][j], dp[i][j_pre] + (j_pre + aa[i]) / M);
        }
    }
