...
//    debug!(H, W, N, board);
    ...
    let mut milestones: Vec<(i32, i32)> = vec![(0, 0); N + 1];
    for i in 0..H {
        for j in 0..W {
            let x = board[i][j];
            if x == 'S' {
                milestones[0] = (i as i32, j as i32);
                board[i][j] = '.';
            }
            if let Some(x) = x.to_digit(10) {
                milestones[x as usize] = (i as i32, j as i32);
                board[i][j] = '.';
            }
        }
    }
//    debug!(milestones);
    ...
    let bfs = |pos_s: (i32, i32), pos_g: (i32, i32)| {
        ...
        while let Some(pos) = queue.pop_front() {
            for &d in DIRECTIONS.iter() {
                ...
                if board[pos_new.0 as usize][pos_new.1 as usize] == 'X' { continue; }
    ...
    let res: usize = milestones.windows(2)
        .map(|w| bfs(w[0], w[1]))
        .sum();
