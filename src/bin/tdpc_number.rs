...
//    debug!(D, N);
    ...
    let mut dp = vec_md![0; N.len() + 1; D; 2];
    dp[0][0][0] = 1;
    ...
    for (i, num) in N.iter().map(to_digit).enumerate() {
        for j in 0..D {
            for &smaller in [false, true].iter() {
                let ub = if smaller { 9 } else { num };
                for l in 0..ub + 1 {
                    let j_next = (j + l) % D;
                    let smaller_next = smaller || l < ub;
                    dp[i + 1][j_next][smaller_next as usize] += dp[i][j][smaller as usize];
                    dp[i + 1][j_next][smaller_next as usize] %= M;
                }
            }
        }
    }
    let res = dp[N.len()][0][0] + dp[N.len()][0][1] - 1;
